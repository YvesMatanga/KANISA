function [xopt,fopt,perf_options] = ABB_PSO_SOLVE(f,xl,xu,options)
...
%% Calculate Upper Bound   
   if options.ub_options.maxIter > 1
        [Node.xopt,Node.ub] =  pso_solver(f,Node.xl,Node.xu,options.ub_options);
   else
        [Node.xopt,Node.ub] =  multistart_search(f,Node.xl,Node.xu,options.ub_options);
   end
...
%% Update Problem Upper bound
    if Node.ub < fopt
        ...
        if StallIter > maxStallIter
            options.ub_options.maxIter = 1;
        end
    end    
...
%% Calculate Lower Bound
    ...
    if ~isempty(find(Node.alpha == Inf,1)) 
        ...
        [lb_xopti,Node.lb,exitFlag] = fmincon(func_i,Node.xopt,[],[],[],[],Node.xl,Node.xu,[],options.lbSolverOptions);%calculate lower bound
    end
...
%% Branching Decision
 ...
 if Node.lb > Node.If.inf
   ...
   if LBk == -inf
       error('problem cannot be solved using branch and bound framework...')
   end
