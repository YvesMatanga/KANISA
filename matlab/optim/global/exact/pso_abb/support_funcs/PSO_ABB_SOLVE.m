function [xopt,fopt,perf_options] = PSO_ABB_SOLVE(f,xl,xu,options)
...
perf_options.cvxPruningCount = 0;%Initialise runing count by convexity
...
lbubGvEps = options.eps*100;%Non-sensiscal violation lower bound above upper bound
%% Initialisation 
...
%% Global optimum search
 if isPSO_based == 1
    [cxopt,cub] =  pso_solver(f,xl,xu,options.ub_options);
...
%% Search Strategy
 [curId,isPSO_based] = searchStrategy(BNodes,options.search,xopt);
 ...
 if isPSO_based == 1
    ...
    [Node.xopt,Node.ub] =  fmincon(f,x0i,[],[],[],[],Node.xl,Node.xu,[],options.ubSolverOptions);
...
%% Search Strategies
function [id,isPSO_based] = searchStrategy(BNodesIn,option,xOpt)
   if strcmp(option,'bfs') == 1%default search strategy
       [id,isPSO_based] = searchByPSOLead(BNodesIn,xOpt);
   else
...
function [id,cisPSO_based] = searchByPSOLead(BNodesIn,xOpt)
   N = length(BNodesIn);
   id = -1;
   if N == 1
       id = 1;
       ...
       for k=1:N%go through every node
           Bk = BNodesIn{k};           
           if inBounds(Bk.xl,Bk.xu,xOpt) == 1
               id = k;
               cisPSO_based = 1;
               return;
           end
       end       
       if id == -1 %solution not within nodes
           id = searchByAllBounds(BNodesIn);%use best first strategy
           cisPSO_based = 0;
           return;
       end
   end
